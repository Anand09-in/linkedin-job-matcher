...
- Vite + React + TS scaffold; Tailwind v4 + hand-rolled shadcn-style primitives (`class-variance-authority` + `cn()`, the same pattern shadcn's CLI generates — written directly rather than pulled via the CLI/registry, to avoid a live-registry dependency for a one-time scaffold); TanStack Query + Zustand; React Router v7.
- Generated API client: `openapi-typescript` turns Phase 7's `openapi.json` into `src/api/schema.d.ts`, consumed via `openapi-fetch` (`src/api/client.ts`) — every hook in `src/api/hooks/` is fully typed against the real backend contract, so a response-shape change is a compile error here, not a silent runtime bug.
- Pages: Job Results (filter/sort/paginate, bulk delete by date, **filter by pipeline**), Job Detail (score breakdown, on-demand feature triggers with loading/cached states, all 7 Phase 6/7 features), Resume library (upload/list/delete multiple resumes, each named), Pipelines manager (create/edit pipelines — name, bound resume, site, query, locations, filters, batch size, threshold overrides, enable/disable, schedule) + per-pipeline run status/history + rejected-jobs view, Tracker (status funnel board), Settings (global LLM selection only — the UI surface for Phase 5's backend capability).
- Dockerized: Vite dev server container for dev (now with a live source bind mount + HMR — added mid-phase once the original build-time-only image made every one-line edit require a full rebuild), Nginx-served static build for prod (FR-9.2, `Dockerfile.prod` + `nginx.conf`, SPA-fallback routing for client-side routes).
- **A real, load-bearing bug found only by actually running this in a browser** (not by `tsc`/`vite build`, which both passed the whole time): the API had no CORS middleware at all. `localhost:5173` and `localhost:8000` are different origins by port alone, so every request from a real user's browser would have been blocked — not just an artifact of headless testing. Fixed with `CORSMiddleware` in `main.py`, origins configurable via `CORS_ALLOWED_ORIGINS`.
- Verified with a headless-Chromium walkthrough (Playwright, already installed in the `worker` image for LinkedIn scraping — reused here rather than adding a browser to the frontend image) driving all 6 pages against the real backend: job list/detail, triggering an on-demand feature end-to-end (including serving from cache), pipeline create dialog, resume library, tracker funnel, settings update — zero console errors on the final run.
